...
| 🖱️ 一键跳转代码行 | 点击任意节点，直接跳转至编辑器对应代码行 | 🔜 V4 开发中 |
| 🤖 AI 辅助修复 | 每个错误卡片内置 AI 修复入口，自动生成修复方案 | 🔜 V4 开发中 |
...
### 方式三：接入你的编辑器
支持 VS Code / Monaco Editor 接入，完整协议见 [接入指南](./docs/integration.md)。
...
完整协议规范：[postMessage API 文档](./docs/postmessage-api.md)
...
#### ⚠️ 已知问题 / 限制
- 🔜 **Demo 是手动模拟的** —— 真实接入编辑器功能在 V4 开发
- 🔜 **不支持超大型项目**（>1000 个节点会卡） —— V5 三维化后会优化
- 🔜 **不支持移动端触控** —— 正在开发中
- 💬 **postMessage API 还在 Draft 阶段** —— 协议可能变动，接入前请先 Issue 讨论

### 🔜 未来规划
| 版本 | 核心能力 | 用户价值 |
|------|----------|----------|
| V4 | 真实 AST 解析 + 编辑器接入 | 你的代码，自动长成对应的树 |
| V5 | WebGL 3D 渲染，自由旋转缩放 | 沉浸式漫游你的代码架构 |
| V6 | 运行时性能可视化 | 树随 CPU/内存负载动态「呼吸」 |
| V7 | AI 对话联动 | 提问问题 → 对应分枝自动高亮 |
